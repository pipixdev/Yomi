...
- Then read `Yomi/BookshelfView.swift`. It is the main user-facing screen for importing, listing, opening, rebuilding, and removing books. Book cards contain separate open and action buttons. On iOS, `BookActionsMenu` bridges a native `UIButton` / `UIMenu`; UIKit owns placement, animation, outside-tap dismissal, and accessibility. `BookshelfInteraction` keeps browsing/menu/reading mutually exclusive. The button's `willDisplayMenuFor` enters menu state, and only the dismissal animator's completion leaves it and executes a selected action. Cover taps during both menu display and dismissal are ignored. Menus are built on demand from current translation/import state, not on every progress update. The custom overlay, anchor preferences, and menu layout are removed. Non-iOS builds use SwiftUI `Menu`.
...
- Reader performance regression checks live in `Tests/reader-script-checks.mjs`, `Tests/analysis-scroll-checks.mjs`, `Tests/TranslationMemoryCacheChecks.swift`, `Tests/ReaderPersistenceChecks.swift`, and `Tests/BookshelfInteractionChecks.swift`; see `docs/reader-performance.md` for commands, scope, and remaining profiling needs.
